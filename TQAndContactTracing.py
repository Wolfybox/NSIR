...
def get_global_pos_n(node_i):
    base_neighbors = np.array([i for i in base_graph.neighbors(node_i)])
    neighbors_states = state_list[base_neighbors]
    global_P_neighbors = len(np.where(neighbors_states == 3))
    return global_P_neighbors
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_name', default=r'Testing_Quarantine_ContactTracing',
                        help='name of log of current simulation')
    ...
    parser.add_argument('--contact_trace_rate', type=float, default=1.0, help='ratio of contact tracing')
    ...
    log_name = f'{config.log_name}_p{config.global_ratio}_theta{config.mass_testing_rate}_ph{config.contact_trace_rate}.txt'
    log_lines = []
    t_cur = 0
    while t_cur < t_max:
        ...
        # Contact Tracing
        p_I2P = np.array([min([1.0, theta + ph * get_global_pos_n(node)]) for node in I_node])
        p_I2PRF = np.array([gamma + mu] * I_num) + p_I2P
        p_P2RF = np.array([gamma_p + mu_p] * P_num)
        prob_arr = np.concatenate([p_S2E, p_E2I, p_I2PRF, p_P2RF], axis=0)

        # calculate probability distribution for sampling
        ...
        if chosen_node_state == 0:
            ...
            target_state = \
                np.random.choice([3] * I_num + [4] * I_num + [5] * I_num, replace=True, size=1, p=I_trans_prob)[0]
        else:
